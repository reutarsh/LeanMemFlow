...
| `pe_timedatestamp_utc` | string | **Same stamp as UTC date** (`YYYY-MM-DD HH:MM:SS`). Derived from `pe_timedatestamp` when that value is positive; blank for empty/`0`/invalid. |
...
**Dump PE enrichment:** when `run_extract` passes `--dump-path`, the dlls extractor opens the dump via the `memprocfs` Python package, reads mapped PE headers for PIDs present in the CSV, and fills empty `entry_point_rva` / `entry_point` / `pe_timedatestamp` / `pe_checksum` (and derives `pe_timedatestamp_utc`). If `memprocfs` is missing or enrichment fails, CSV output is still written; those fields stay empty. Without a dump path, behavior is CSV-only (plus `module_type` from Name prefixes); `pe_timedatestamp_utc` is still derived when a timestamp is present in the source CSV.
...
| dlls | `dlls.csv` | forensic_csv (modules.csv) + optional dump PE | pid, module_name, module_type, base_address, entry_point, entry_point_rva, pe_timedatestamp, pe_timedatestamp_utc, pe_checksum |
